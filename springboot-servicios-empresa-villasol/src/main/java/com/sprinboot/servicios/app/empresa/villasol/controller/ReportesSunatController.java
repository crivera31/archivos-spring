...
import com.sprinboot.servicios.app.empresa.villasol.dto.PlanContableDto;
...
import com.sprinboot.servicios.app.empresa.villasol.funciones.JasperSpring;
import com.sprinboot.servicios.app.empresa.villasol.service.AsientoServiceInterface;
import com.sprinboot.servicios.app.empresa.villasol.service.ILibroMayorService;
import com.sprinboot.servicios.app.empresa.villasol.service.RegistroLibrosServiceInterface;
import com.sprinboot.servicios.app.empresa.villasol.service.ReporteLIbroDiarioService;
...
public class ReportesSunatController {
	...
	@GetMapping(value="/registro-auxiliar-compras/{idperiodo}/{anio}/{codmes}/{codlibro}/{codCtaParametro}/{codClasificacion}")
    public ResponseEntity<?> listRegistroAuxiliaresCompras(@PathVariable Integer idperiodo, @PathVariable String anio,@PathVariable Integer codmes,@PathVariable String codlibro,@PathVariable String codCtaParametro,@PathVariable String codClasificacion) {
		...
		try {
			for (int i = 0; i < codDocumentos.length; i++) {
				...
				rs= registroLibrosServiceInterface.listarRegistroAuxiliares(codDocumentos[i], idperiodo, codmes, anio, 2,codlibro,codCtaParametro,"03","0001");
			    if(rs!=null) {
	...
	@GetMapping(value="/registro-auxiliar-ventas/{idperiodo}/{anio}/{codmes}/{codlibro}/{codCtaParametro}/{codClasificacion}")
    public ResponseEntity<?> listRegistroAuxiliarVentas(@PathVariable Integer idperiodo, @PathVariable String anio,@PathVariable Integer codmes,@PathVariable String codlibro,@PathVariable String codCtaParametro,@PathVariable String codClasificacion) {
		Map<String, Object> response = new HashMap<>();
		...
		try {
			for (int i = 0; i < codDocumentos.length; i++) {
				...
				RegistroComprasDetDto rs= new RegistroComprasDetDto();
				rs=null;
				rs= registroLibrosServiceInterface.listarRegistroAuxiliares(codDocumentos[i], idperiodo, codmes, anio, 2,codlibro,codCtaParametro,"04","0001");
			    if(rs!=null) {
					listRegistroComprasDetDto.add(rs);
			    }
			}
    ...
    public ResponseEntity<?> registroComprasPdf(@PathVariable Integer idperiodo, @PathVariable String anio,@PathVariable Integer codmes,@PathVariable String codlibro,@PathVariable String codCtaParametro,@PathVariable String codClasificacion,@PathVariable String nommes) {
		...
		try {
		 ...
		 parameters.put("constructora", "TECNOLOGIA Y CONSTRUCCION VILLASOL S.A.C.");
		 parameters.put("direccion", "BRASIL NRO. 144 DPTO. 3");
		 parameters.put("ejercicio", "EJERCICIO 2020");
		 parameters.put("ruc", "20555172746");
