...
import com.sprinboot.servicios.app.empresa.constructores.solid.DescuadradoDolares;
import com.sprinboot.servicios.app.empresa.constructores.solid.DescuadradoDolaresSoles;
import com.sprinboot.servicios.app.empresa.constructores.solid.DescuadradoSoles;
import com.sprinboot.servicios.app.empresa.constructores.solid.FilaConsistencia;
...
public class AsientoService extends ObjectService<Asiento, AsientoDao > implements AsientoServiceInterface{
	...
	public Asiento save(Asiento request) {
		 ...
		 asiento.setNumComprobante(request.getNumComprobante());
	...
	public Asiento findUltimoComprobante(String anio) {
		 List<Object[]> results  = asientoDao.listultimocomprobante(anio);
		  Asiento asiento = new Asiento();
		  if (results.size()>0) {
			  asiento.setNumComprobante(Integer.parseInt(results.get(results.size()-1)[0].toString())) ;
			 return asiento ;
		  ...
		  if (results.size()>0) {
			   results.stream().forEach(object->{
		        	  	Asiento a = new Asiento();  
		        	  		  a.setNumComprobante(Integer.parseInt(object[0].toString()));
		        	  		  a.setAnio(anio);
				        	  lstAsiento.add(a);
