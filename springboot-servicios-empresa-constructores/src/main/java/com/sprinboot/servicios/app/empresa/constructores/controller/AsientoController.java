...
public class AsientoController {
	...
	@GetMapping(value="/ultimo-comprobante/{anio}")
	public ResponseEntity<?> ultimoComprobante(@PathVariable("anio") String anio){
		Map<String, Object> response = new HashMap<>();
		Asiento asiento = null;
		Integer numUltimoAsiento = 0;
		try {
			asiento = asientoService.findUltimoComprobante(anio);
			if (asiento==null) {
				numUltimoAsiento = 0;
			}else {
				if (asiento.getNumComprobante()==null) {
					...
					numUltimoAsiento = asiento.getNumComprobante();
				}
			}
		} catch (Exception e) {
             return new ResponseEntity<Object>(HttpStatus.INTERNAL_SERVER_ERROR);
		}	
		response.put("numUltimoComprobante",numUltimoAsiento );
		return new ResponseEntity<Map<String, Object>>(response, HttpStatus.OK);
	}
