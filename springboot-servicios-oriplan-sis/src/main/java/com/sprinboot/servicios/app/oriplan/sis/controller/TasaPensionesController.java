...
@RestController
@RequestMapping("tasa-pensiones")
public class TasaPensionesController {
	
	
	@Autowired
	TasaPensionesServiceInterface service;
	
	@GetMapping(value="/all")
    public ResponseEntity<?> segmentosAll() {
		
	     Map<String, Object> response = new HashMap<>();
		 List<TasasPensiones> lista =  new ArrayList<>();
      try {
    	  lista =  service.listarTasasPensiones();
