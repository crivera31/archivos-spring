...
public class PlanillaController {
    ...
    @GetMapping(value = "/borrar/{idDatosGenerales}/{nomSemana}")
    public ResponseEntity<?> borrar(@PathVariable Integer idDatosGenerales, @PathVariable String nomSemana) {
        List<PlanillaRest> lstPlanillaRest = new ArrayList<>();
        Map<String, Object> response = new HashMap<>();
        List<PlanillaExcel> lista = new ArrayList<>();
        DatosGenerales d = datosService.findByIdCommon(idDatosGenerales);
        
        try {
        	lista = service.getPlaniillita(nomSemana.trim(), d.getNomObra().trim());
        	if (lista.size()>0) {
				lista.stream().forEach( p->{
					p.setEnabled(0);
					service.saveCommon(p);
				});
			}
        } catch (Exception e) {
